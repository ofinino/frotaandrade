...
- `asset_url()` (`Core.php`) gera caminho absoluto a partir da raiz do domínio (`/uploads/...`) quando `config.php` não tem `base_url` — hoje não tem. Isso quebra (404) sempre que o app roda numa subpasta (ex.: `/frotaandrade/`) em vez da raiz do domínio, porque o navegador resolve `/uploads/...` a partir de `http://host/`, não de `http://host/frotaandrade/`. Confirmado ao vivo com a folha de fontes local. Todo link de anexo (`AnexosModel` + `asset_url()`) provavelmente tem o mesmo problema — ao mexer nisso, considerar configurar `base_url` no `config.php` de cada ambiente ou usar caminho relativo simples em vez de `asset_url()`.
